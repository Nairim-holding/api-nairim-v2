...
export class CenterService {
  ...
  static async getCenters(params: any = {}, company_id: string) {
    try {
      ...
      return { data: centers, count: total, totalPages: Math.ceil(total / take), currentPage: page };
    } catch (error: any) {
      throw new Error(`Falha ao buscar centros: ${error.message}`);
    }
  }

  static async getCenterById(id: string, company_id: string) {
    try {
      const center = await prisma.center.findFirst({ where: { id, company_id, deleted_at: null } });
      if (!center) throw new Error('Center not found');
      return center;
    } catch (error: any) { throw error; }
  }

  static async createCenter(data: any, company_id: string) {
    try {
  ...
  static async updateCenter(id: string, data: any, company_id: string) {
    try {
      const existing = await prisma.center.findFirst({ where: { id, company_id, deleted_at: null } });
  ...
  static async deleteCenter(id: string, company_id: string) {
    try {
      const center = await prisma.center.findFirst({ where: { id, company_id, deleted_at: null } });
      if (!center) throw new Error('Center not found or already deleted');

      const hasTransactions = await prisma.transaction.findFirst({ where: { center_id: id, company_id, deleted_at: null } });
      if (hasTransactions) throw new Error('Nao e possivel excluir o centro pois existem lancamentos relacionados.');

      await prisma.center.update({ where: { id }, data: { deleted_at: new Date() } });
      return center;
    } catch (error: any) { throw error; }
  }

  static async restoreCenter(id: string, company_id: string) {
    try {
      const center = await prisma.center.findFirst({ where: { id, company_id } });
      if (!center) throw new Error('Center not found');
      if (!center.deleted_at) throw new Error('Center is not deleted');
      return await prisma.center.update({ where: { id }, data: { deleted_at: null } });
    } catch (error: any) { throw error; }
  }
  ...
  static async quickCreate(data: { name: string; type: 'INCOME' | 'EXPENSE' }, company_id: string) {
    try {
      ...
      throw new Error(`Falha ao criar centro rapido: ${error.message}`);
    }
  }

  static async getCenterFilters(company_id: string) {
    try {
      const where: any = { company_id, deleted_at: null };
      const centers = await prisma.center.findMany({ where, select: { name: true }, orderBy: { name: 'asc' } });
