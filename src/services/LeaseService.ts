      ...
      const lease = await prisma.$transaction(async (tx: any) => {
        ...
        const newLease = await tx.lease.create({
          data: {
            ...
            guarantors: data.guarantors && Array.isArray(data.guarantors) ? data.guarantors : null,
      ...
      const lease = await prisma.$transaction(async (tx: any) => {
        ...
        const updatedLease = await tx.lease.update({
          ...
          data: {
            ...
            guarantors: data.guarantors !== undefined ? (Array.isArray(data.guarantors) ? data.guarantors : null) : existing.guarantors,
